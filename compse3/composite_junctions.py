...
class SE3CompositeJunction:
    ...
    def build_transforms_iterative_correction(self, full_excess_dynamic_coordinates: np.ndarray) -> np.ndarray:
        ...
        for l in range(len(self.X0)):
            ...
            list_dsecs[l] = so3.X2g(excess_dynamic_coordinates[l])
        ...
        for l in range(len(self.X0)):
            ...
            const_vec[3:] -= A[3:,:3] @ list_Phidc[l] + A[3:,3:] @ excess_dynamic_coordinates[l,3:]
            
        corrected_excess_coordinates = so3.g2X(so3.se3_inverse(list_accu_static_comp[0]) @ so3.se3_inverse(self.pose1) @ self.pose2 @ so3.se3_inverse(self.tail_static_junction))
